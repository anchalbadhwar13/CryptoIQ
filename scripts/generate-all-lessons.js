 ...
 * This will test the API connection and generate content for all modules
 * 
 * Usage: node scripts/generate-all-lessons.js
...
async function main() {
  ...
  if (!apiKey) {
    ...
    console.log('   1. Run: npm run setup:api-key')
    console.log('   2. Or manually create .env.local file with: GEMINI_API_KEY=your_api_key_here')
  ...
  results.forEach(lesson => {
    if (lesson.error) {
      ...
      console.log(`    - Videos: ${(lesson.youtubeVideos || []).length}`)
      console.log(`    - Key Points: ${(lesson.keyPoints || []).length}`)
    }
  })
